...
#
# Scans the HF source files to produce a list of methods and properties callable from javascript.
# - Runs doxygen in <root dir>/docs to produce html + xml documentation
# - Loads + traverses the (generated) xml documentation, looking for constructs that can be callable
# from javascript (public slots, signals, Q_INVOKABLEs, etc).
# - Stores this in data structures that can be used to generate a) a comprehensive textual list of 
# the api (dump_api.py), b) a mock js api with jsdoc type annotations, c) fully linked markdown pages
# usable by docs.highfidelity.com.
#
# Additional features:
# - Builds a complete index of the c++ codebase (thanks to doxygen). Contains type information, call
# and reference graphs., etc. Could be converted into some other, potentially more useful tool if so desired.
# - scanner is fully multithreaded, and the python component runs in a few seconds (after doxygen completes).
#
